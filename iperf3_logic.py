...
class IPerf3Logic:
    def __init__(self, client_ip, scenario_name, test_group_name=None, rtt_suffix="", output_format=None, platform_override=None):
        ...
        if self.test_group_name:
            self.output_prefix = f"{self.test_group_name}_{self.scenario_name}{self.rtt_suffix}{platform_suffix}_{self.timestamp_str}"
        else:
            self.output_prefix = f"{self.scenario_name}{self.rtt_suffix}{platform_suffix}_{self.timestamp_str}"
            
        self.log_file = f"/home/aphillips/Projects/Access_Architecture/LLD_TEST_CLT/logs/iperf3_{self.test_id}.log"
        self.log_rotator = LogRotator(self.log_file)
        
        # Define scenario commands based on output format and platform
        json_ext = "-J" if self.output_format == "json" else "-i 1"
        file_ext = "json" if self.output_format == "json" else "txt"
        ...
        if is_macos:
            ...
            self.scenario_commands = {
                "US_Combined": [
                    f"iperf3-darwin -c {self.server_ip} --apple-quic -t 30 -P 4 -p 5201 {json_ext} > US_{group_name}_Combined_4QUIC_CL.{file_ext} &",
                    f"iperf3-darwin -c {self.server_ip} --apple-quic -t 30 -P 1 -p 5207 --apple-l4s {json_ext} > US_{group_name}_Combined_1QUIC_LL.{file_ext} &",
                    f"iperf3-darwin -c {self.server_ip} -u -b 0.8192M -t 30 -p 5209 {json_ext} > US_{group_name}_Combined_1UDP_CL.{file_ext} &",
                    f"iperf3-darwin -c {self.server_ip} -u -b 0.8192M --dscp 45 -t 30 -p 5211 {json_ext} > US_{group_name}_Combined_1UDP_LL.{file_ext} &",
                    "wait"
                ],
                "US_Classic_Only": [
                    f"iperf3-darwin -c {self.server_ip} --apple-quic -t 30 -P 4 -p 5201 {json_ext} > US_{group_name}_Classic_Only_4QUIC_CL.{file_ext} &",
                    f"iperf3-darwin -c {self.server_ip} -u -b 0.8192M -t 30 -p 5209 {json_ext} > US_{group_name}_Classic_Only_1UDP_CL.{file_ext} &",
                    "wait"
                ],
                "US_LL_Only": [
                    f"iperf3-darwin -c {self.server_ip} --apple-quic -t 30 -P 1 -p 5207 --apple-l4s {json_ext} > US_{group_name}_LL_Only_1QUIC_LL.{file_ext} &",
                    f"iperf3-darwin -c {self.server_ip} -u -b 0.8192M --dscp 45 -t 30 -p 5211 {json_ext} > US_{group_name}_LL_Only_1UDP_LL.{file_ext} &",
                    "wait"
                ],
                "DS_Combined": [
                    f"iperf3-darwin -c {self.client_ip} --apple-quic -t 30 -P 4 -p 5201 {json_ext} > DS_{group_name}_Combined_4QUIC_CL.{file_ext} &",
                    f"iperf3-darwin -c {self.client_ip} --apple-quic -t 30 -P 1 -p 5207 --apple-l4s {json_ext} > DS_{group_name}_Combined_1QUIC_LL.{file_ext} &",
                    f"iperf3-darwin -c {self.client_ip} -u -b 0.8192M -t 30 -p 5209 {json_ext} > DS_{group_name}_Combined_1UDP_CL.{file_ext} &",
                    f"iperf3-darwin -c {self.client_ip} -u -b 0.8192M --dscp 45 -t 30 -p 5211 {json_ext} > DS_{group_name}_Combined_1UDP_LL.{file_ext} &",
                    "wait"
                ],
                "DS_Classic_Only": [
                    f"iperf3-darwin -c {self.client_ip} --apple-quic -t 30 -P 4 -p 5201 {json_ext} > DS_{group_name}_Classic_Only_4QUIC_CL.{file_ext} &",
                    f"iperf3-darwin -c {self.client_ip} -u -b 0.8192M -t 30 -p 5209 {json_ext} > DS_{group_name}_Classic_Only_1UDP_CL.{file_ext} &",
                    "wait"
                ],
                "DS_LL_Only": [
                    f"iperf3-darwin -c {self.client_ip} --apple-quic -t 30 -P 1 -p 5207 --apple-l4s {json_ext} > DS_{group_name}_LL_Only_1QUIC_LL.{file_ext} &",
                    f"iperf3-darwin -c {self.client_ip} -u -b 0.8192M --dscp 45 -t 30 -p 5211 {json_ext} > DS_{group_name}_LL_Only_1UDP_LL.{file_ext} &",
                    "wait"
                ]
            }
            # macOS uses different server ports
            self.server_ports = [5201, 5207, 5209, 5211]
        else:
            # Linux commands using standard iperf3 with TCP and Prague
            self.scenario_commands = {
                "US_Combined": [
                    f"iperf3 -c {self.server_ip} -t 30 -p 5210 -P 4 -C cubic {json_ext} > US_{group_name}_Combined_4TCP_CL.{file_ext} &",
                    f"iperf3 -c {self.server_ip} -t 30 -p 5205 -P 1 -C prague {json_ext} > US_{group_name}_Combined_1TCP_LL.{file_ext} &",
                    f"iperf3 -c {self.server_ip} -t 30 -u -b 0.82M -p 5202 -P 1 {json_ext} > US_{group_name}_Combined_1UDP_CL.{file_ext} &",
                    f"iperf3 -c {self.server_ip} -t 30 -u -b 0.82M -p 5206 -P 1 --dscp 45 {json_ext} > US_{group_name}_Combined_1UDP_LL.{file_ext} &",
                    "wait"
                ],
                "US_Classic_Only": [
                    f"iperf3 -c {self.server_ip} -t 30 -p 5210 -P 4 -C cubic {json_ext} > US_{group_name}_Classic_Only_4TCP_CL.{file_ext} &",
                    f"iperf3 -c {self.server_ip} -t 30 -u -b 0.82M -p 5202 -P 1 {json_ext} > US_{group_name}_Classic_Only_1UDP_CL.{file_ext} &",
                    "wait"
                ],
                "US_LL_Only": [
                    f"iperf3 -c {self.server_ip} -t 30 -p 5205 -P 1 -C prague {json_ext} > US_{group_name}_LL_Only_1TCP_LL.{file_ext} &",
                    f"iperf3 -c {self.server_ip} -t 30 -u -b 0.82M -p 5206 -P 1 --dscp 45 {json_ext} > US_{group_name}_LL_Only_1UDP_LL.{file_ext} &",
                    "wait"
                ],
                "DS_Combined": [
                    f"iperf3 -c {self.client_ip} -t 30 -p 5210 -P 4 -C cubic {json_ext} > DS_{group_name}_Combined_4TCP_CL.{file_ext} &",
                    f"iperf3 -c {self.client_ip} -t 30 -p 5205 -P 1 -C prague {json_ext} > DS_{group_name}_Combined_1TCP_LL.{file_ext} &",
                    f"iperf3 -c {self.client_ip} -t 30 -u -b 0.82M -p 5202 -P 1 {json_ext} > DS_{group_name}_Combined_1UDP_CL.{file_ext} &",
                    f"iperf3 -c {self.client_ip} -t 30 -u -b 0.82M -p 5206 -P 1 --dscp 45 {json_ext} > DS_{group_name}_Combined_1UDP_LL.{file_ext} &",
                    "wait"
                ],
                "DS_Classic_Only": [
                    f"iperf3 -c {self.client_ip} -t 30 -p 5210 -P 4 -C cubic {json_ext} > DS_{group_name}_Classic_Only_4TCP_CL.{file_ext} &",
                    f"iperf3 -c {self.client_ip} -t 30 -u -b 0.82M -p 5202 -P 1 {json_ext} > DS_{group_name}_Classic_Only_1UDP_CL.{file_ext} &",
                    "wait"
                ],
                "DS_LL_Only": [
                    f"iperf3 -c {self.client_ip} -t 30 -p 5205 -P 1 -C prague {json_ext} > DS_{group_name}_LL_Only_1TCP_LL.{file_ext} &",
                    f"iperf3 -c {self.client_ip} -t 30 -u -b 0.82M -p 5206 -P 1 --dscp 45 {json_ext} > DS_{group_name}_LL_Only_1UDP_LL.{file_ext} &",
                    "wait"
                ]
            }
            # Linux uses different server ports
            self.server_ports = [5202, 5205, 5206, 5210]
    ...
    def setup_ssh_keys(self):
        ...
        # Skip SSH key setup for macOS and use password auth
        if hasattr(self, 'platform_override') and self.platform_override == 'macos':
            self.logger.info("Using password authentication for macOS client")
            return True
    ...
    def run_scenario(self, iteration, total_iterations):
        ...
        # Determine which host to run commands on based on scenario
        ...
        try:
            ...
            for cmd in commands:
                ...
                self.logger.info(f"Starting background: {full_cmd}")
            ...
            self.logger.info("Waiting for all iPerf3 commands to complete...")
            ...
            self.logger.info(f"iPerf3 completed successfully, output: {output_dir}")
            return True
