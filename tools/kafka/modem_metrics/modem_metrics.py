...
vCMTS Snapshot options:
    ...
    --cm-mac MAC           Filter by CM MAC address
    --test-duration SEC    Actual test duration in seconds (e.g. 60)
    --utc                  Input timestamps are ByteBlower UTC; auto-convert +6h to DB time
...
POLL_INTERVAL_SEC = 60  # wider window to capture burst ramp-up/tail across 30s polling intervals
...
def vcmts_snapshot(conn, args):
    ...
    for table in VCMTS_METRICS_TABLES:
        ...
        if group_keys is None:
            ...
            # row layout: (*group_keys, timestamp, value)
            start_map = {row[:n_keys]: (row[n_keys], row[-1]) for row in start_rows}  # key -> (timestamp, value)
            end_map = {row[:n_keys]: (row[n_keys], row[-1]) for row in end_rows}

            all_keys = sorted(set(start_map) | set(end_map))
            delta_headers = group_keys + ["scenario_start", "start_value", "scenario_end", "end_value", "delta"]
            delta_rows = []
            for key in all_keys:
                s_ts, sv = start_map.get(key, (None, None))
                e_ts, ev = end_map.get(key, (None, None))
                delta = (ev or 0) - (sv or 0)
                delta_rows.append(list(key) + [s_ts, sv, e_ts, ev, delta])
    ...
    for table in CM_VALUE_COLS:
        ...
        start_map = {row[0]: (row[1], row[2:]) for row in start_rows}  # sfid -> (time, values)
        end_map = {row[0]: (row[1], row[2:]) for row in end_rows}

        all_sfids = sorted(set(start_map) | set(end_map))
        delta_headers = ["sfid", "name", "direction", "scenario_start", "scenario_end"]
        for col in val_cols:
        ...
        for sfid in all_sfids:
            ...
            direction = "US" if name.startswith("us") else "DS" if name.startswith("ds") else "?"
            s_time, sv_vals = start_map.get(sfid, (None, None))
            e_time, ev_vals = end_map.get(sfid, (None, None))
            row = [sfid, name, direction, s_time, e_time]
    ...
    # --- Summary Sheet ---
    print("\nGenerating summary...")
    # Determine durations for throughput calculation
    test_duration = getattr(args, 'test_duration', None)
    summary_headers = [
        "direction", "sf_index", "scenario_start", "scenario_end",
        "poll_window_sec", "total_packets", "total_octets", "total_bits",
        "poll_throughput_mbps",
    ]
    if test_duration:
        summary_headers.append("test_throughput_mbps")
    summary_headers += [
    ...
    def _get_delta_and_ts_map(conn, table, mac_filter, since, until):
        """Return {(direction, sf_index): (delta, start_ts, end_ts)} for a single-value vCMTS table."""
        ...
        # row: (cm_mac_addr, direction, sf_index, timestamp, value)
        start_map = {(r[1], r[2]): (r[3], r[-1]) for r in start_rows}
        end_map = {(r[1], r[2]): (r[3], r[-1]) for r in end_rows}
        result = {}
        for k in set(start_map) | set(end_map):
            s_ts, sv = start_map.get(k, (None, 0))
            e_ts, ev = end_map.get(k, (None, 0))
            result[k] = ((ev or 0) - (sv or 0), s_ts, e_ts)
        return result

    packets_map = _get_delta_and_ts_map(conn, "qos_service_flow_packets", mac_filter, args.since, args.until)
    octets_map = _get_delta_and_ts_map(conn, "qos_service_flow_octets", mac_filter, args.since, args.until)
    aqm_dropped_map = _get_delta_and_ts_map(conn, "dp_flow_aqm_dropped_packets", mac_filter, args.since, args.until)
    aqm_marked_map = _get_delta_and_ts_map(conn, "dp_flow_aqm_marked_congested_packets", mac_filter, args.since, args.until)
    sanctioned_map = _get_delta_and_ts_map(conn, "dp_flow_sanctioned_packets", mac_filter, args.since, args.until)
    ksamis_dropped_map = _get_delta_and_ts_map(conn, "ksamis1_delta_packets_dropped", mac_filter, args.since, args.until)
    ...
    all_flows = sorted(set(packets_map) | set(octets_map) | set(aqm_dropped_map))
    for direction, sf_index in all_flows:
        k = (direction, sf_index)
        pkts, s_ts, e_ts = packets_map.get(k, (0, None, None))
        octets = octets_map.get(k, (0, None, None))[0]
        total_bits = octets * 8 if octets else 0

        # Poll-to-poll throughput (uses actual DB sample timestamps)
        if s_ts and e_ts:
            try:
                poll_sec = (e_ts - s_ts).total_seconds()
            except Exception:
                poll_sec = duration_sec
        else:
            poll_sec = duration_sec
        if poll_sec and poll_sec > 0 and octets:
            poll_mbps = round(total_bits / poll_sec / 1_000_000, 4)
        else:
            poll_mbps = 0.0
        ...
        row = [
            direction, sf_index, s_ts, e_ts,
            round(poll_sec, 3) if poll_sec else None,
            pkts, octets, total_bits,
            poll_mbps,
        ]
        if test_duration:
            test_mbps = round(total_bits / test_duration / 1_000_000, 4) if octets else 0.0
            row.append(test_mbps)
        row += [
            avg_lat, p99_lat, max_lat,
            aqm_dropped_map.get(k, (0,))[0], aqm_marked_map.get(k, (0,))[0],
            sanctioned_map.get(k, (0,))[0], ksamis_dropped_map.get(k, (0,))[0],
        ]
        summary_rows.append(row)

    total_rows += _write_sheet(wb, outdir, "summary", summary_headers, summary_rows)

    # --- Per-interval throughput detail sheet ---
    print("\nGenerating throughput detail...")
    if mac_filter:
        detail_sql = (
            "WITH samples AS ("
            "  SELECT direction, sf_index, \"timestamp\", value,"
            "    LAG(value) OVER (PARTITION BY direction, sf_index ORDER BY \"timestamp\") as prev_value,"
            "    value - LAG(value) OVER (PARTITION BY direction, sf_index ORDER BY \"timestamp\") as delta_octets,"
            "    EXTRACT(EPOCH FROM \"timestamp\" - LAG(\"timestamp\") OVER (PARTITION BY direction, sf_index ORDER BY \"timestamp\")) as delta_sec"
            "  FROM qos_service_flow_octets"
            "  WHERE cm_mac_addr = %s"
            "    AND \"timestamp\" >= %s::timestamp - interval '120 seconds'"
            "    AND \"timestamp\" <= %s::timestamp + interval '120 seconds'"
            ") SELECT direction, sf_index, \"timestamp\", prev_value, value, delta_octets, delta_sec,"
            "  CASE WHEN delta_sec > 0 THEN round((delta_octets * 8.0 / delta_sec / 1000000)::numeric, 4) END as interval_mbps"
            " FROM samples WHERE delta_octets IS NOT NULL"
            " ORDER BY \"timestamp\", direction, sf_index"
        )
        cur = conn.cursor()
        cur.execute(detail_sql, [mac_filter, args.since, args.until])
        detail_headers = [desc[0] for desc in cur.description]
        detail_rows = cur.fetchall()
        total_rows += _write_sheet(wb, outdir, "throughput_detail", detail_headers, detail_rows)

        # Print to terminal (only intervals > 1 Mbps)
        print(f"\n  Per-interval throughput (> 1 Mbps):")
        for row in detail_rows:
            if row[7] and row[7] > 1:
                print(f"    {row[2]}  {row[0]:>10} sf{row[1]}  prev={row[3]:,.0f}  curr={row[4]:,.0f}  delta={row[5]:,.0f}  {row[7]:>10} Mbps")

    # --- Style the summary sheet and move to front ---
    ws_summary = wb["summary"]
    if "throughput_detail" in wb.sheetnames:
        ws_detail = wb["throughput_detail"]
        wb.move_sheet(ws_detail, offset=-len(wb.sheetnames) + 2)
    ...
    # Title rows
    note_rows = 5 if test_duration else 3
    ws_summary.insert_rows(1, note_rows)
    ...
    ws_summary.cell(row=2, column=1, value=f"Scenario Window: {args.since} → {args.until}  ({duration_sec:.1f}s)").font = Font(name="Calibri", size=10, italic=True)
    ...
    if test_duration:
        ws_summary.cell(row=3, column=1, value=f"Actual Test Duration: {test_duration}s").font = Font(name="Calibri", size=10, italic=True)
        note_font = Font(name="Calibri", size=9, italic=True, color="666666")
        ws_summary.cell(row=4, column=1,
            value="poll_throughput_mbps = octets×8 / poll_window_sec (time between DB samples, includes idle time before/after burst)"
        ).font = note_font
        ws_summary.cell(row=5, column=1,
            value="test_throughput_mbps = octets×8 / test_duration (actual traffic generator duration). "
                  "Difference is due to: (1) 30s polling granularity capturing idle time in edge intervals, "
                  "(2) DOCSIS/L2 overhead counted in DB but not by traffic generator (~3-5%%)."
        ).font = note_font
    ...
    print(f"\n{'='*80}")
    print(f"  SUMMARY  (scenario: {args.since} -> {args.until}, {duration_sec:.1f}s)")
    if test_duration:
        print(f"  Actual test duration: {test_duration}s")
        print(f"  NOTE: poll_throughput uses DB sample window (includes idle edge intervals)")
        print(f"         test_throughput uses actual test duration (more accurate for burst traffic)")
        print(f"         ~3-5% overhead expected: DB counts DOCSIS/L2 headers not counted by traffic generator")
    print(f"{'='*80}")
...
def main():
    ...
    p_snap.add_argument("--test-duration", type=float, help="Actual test duration in seconds (e.g. 60) for accurate throughput calculation")
    p_snap.add_argument("--utc", action="store_true", help="Input timestamps are ByteBlower UTC; auto-convert +6h to DB time")
        ...
        if args.command == "tables":
            ...
            if args.utc:
                ...
                for field in ("since", "until"):
                    ts_str = getattr(args, field).replace("T", " ")
                    ts = datetime.fromisoformat(ts_str)
                    converted = ts + timedelta(hours=DB_OFFSET_HOURS)
                    setattr(args, field, converted.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                print(f"  UTC -> DB time (+{DB_OFFSET_HOURS}h): {args.since} to {args.until}")
